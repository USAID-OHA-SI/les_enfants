...
### All charts include USAID-specific, Pediatric (<15 years) data, unless noted otherwise. 

To dos:
1. add narratives at the bottom in appendix
2. add UNAIDS data 
...
- The number of new tests was **`r scales::comma(current_HTS_TST)`**, which is a **`r abs(round(change_HTS_TST_prev_quarter, 1))`%** `r ifelse(change_HTS_TST_prev_quarter >= 0, "increase", "decrease")` from last quarter and a **`r abs(round(change_HTS_TST_year_ago, 1))`%** `r ifelse(change_HTS_TST_year_ago >= 0, "increase", "decrease")` from one year ago. The positivity rate was **`r scales::percent(current_Positivity_Rate, accuracy = 0.1)`**. 

- There are **`r scales::comma(current_TX_CURR)`** patients on ART. This is a **`r abs(round(change_TX_CURR_prev_quarter, 1))`%** `r ifelse(change_TX_CURR_prev_quarter >= 0, "increase", "decrease")` from last quarter and a **`r abs(round(change_TX_CURR_year_ago, 1))`%** `r ifelse(change_TX_CURR_year_ago >= 0, "increase", "decrease")` from one year ago. The linkage rate is  **`r scales::percent(abs(round(current_Linkage_rate, 3)), accuracy = 0.1)`**. 

- The viral suppression rate is **`r scales::percent(current_VL_Suppression_Rate, accuracy = 0.1)`**. This is a **`r abs(round(change_VL_Suppression_Rate_prev_quarter, 1))`** percentage point `r ifelse(change_VL_Suppression_Rate_prev_quarter >= 0, "increase", "decrease")` from last quarter and a **`r abs(round(change_VL_Suppression_Rate_year_ago, 1))`** percentage point `r ifelse(change_VL_Suppression_Rate_year_ago >= 0, "increase", "decrease")` from one year ago. 
...
rate_trend_plot <- ggplot(trend_rates, aes(x = time, y = Rate, color = Metric, group = Metric)) +
  ...
  labs(
    ...
    subtitle = 'Pediatrics (<15 years of age)',
    y = "",
    x = "", 
    color = "Metric", 
...
positivity_rate_plot <- ggplot(positivity_rates, aes(x = time, y = Positivity_Rate, color = Metric, group = Metric)) +
  ...
  labs(
    title = 'Trend of HIV Positive Tests',
    subtitle = 'Pediatrics (<15 years of age)',
    y = "",
    x = "", 
    color = "Metric", 
...
```{r, trend plot 2,  warning=FALSE, message=FALSE, fig.width=12, fig.height=6}
##### Trend plot 2
...
plot1_usaid <- ggplot(plot_data, aes(x = indicator, y = total_value, fill = Type)) +
    ...
    labs(
      ...
      subtitle = 'USAID: Pediatrics (<15 years of age)',
      y = "Total Count",
      x = "",
      fill = "Value Type",
...
plot1_cdc <- ggplot(plot_data, aes(x = indicator, y = total_value, fill = Type)) +
    ...
    labs(
      title = 'HTS_TST & HTS_TST_POS',
      subtitle = 'CDC: Pediatrics (<15 years of age)',
      y = "Total Count",
      x = "",
      fill = "Value Type",
      caption = "Positivity Rate is calculated as HTS_TST_POS / HTS_TST"
...
plot_usaid <- ggplot(plot_data, aes(x = indicator, y = total_value, fill = indicator)) +
    ...
    geom_text(aes(label = scales::comma(total_value)), 
              vjust = -0.5, size = 3.5) +
    labs(
      title = paste0(country_selected, ", FY ", fiscal_year_selected, ", Q", quarter_selected, " (USAID)"),
      y = "Total Count",
      x = "",
...
annotation_data <- data_filtered %>%
  ...
  plot_cdc <- ggplot(plot_data, aes(x = indicator, y = total_value, fill = indicator)) +
      ...
      geom_text(aes(label = scales::comma(total_value)), 
                vjust = -0.5, size = 3.5) +
      labs(
        title = paste0(country_selected, ", FY ", fiscal_year_selected, ", Q", quarter_selected, " (CDC)"),
        y = "Total Count",
        x = "",
        caption = "Proxy Linkage Rate calculated as TX_NEW / HTS_TST_POS"
      ) +
...
# Create the updated bar plot
  plot <- ggplot(plot_data, aes(x = indicator, y = total_value, fill = indicator)) +
    ...
    labs(
      title = paste0(country_selected, ", FY ", fiscal_year_selected, ", Q", quarter_selected),
      subtitle = 'Pediatrics (<15 years of age)',
      y = "Total Count",
      x = ""
    ) +
...
combined_plot_3rd95 <- plot_grid(plot_usaid, plot_cdc, ncol = 2)
combined_plot_3rd95
...
create_highlighted_barplot <- function(data, indicator, y_label, title, subtitle, caption) {
  ...
  ggplot(plot_data, aes(x = country_ordered, y = !!sym(indicator), fill = highlight)) +
    ...
    geom_text(
      ...
      aes(label = sprintf("%.3f", !!sym(indicator))),
      vjust = -0.5,
      size = 3,
      color = "black"
    ) +
...
create_highlighted_barplot(
  ...
  title = paste("Positivity Rate by Country for FY", fiscal_year_selected, "Q", quarter_selected),
...
create_highlighted_barplot(
  ...
  y_label = "Linkage Rate",
  title = paste("Linkage Rate by Country for FY", fiscal_year_selected, "Q", quarter_selected),
  subtitle = "Pediatric (<15) Data",
  caption = "Linkage Rate is calculated as TX_NEW / HTS_TST_POS."
)
...
create_highlighted_barplot(
  ...
  y_label = "VL Suppression Rate",
  title = paste("VL Suppression Rate by Country for FY", fiscal_year_selected, "Q", quarter_selected),
  subtitle = "Pediatric (<15) Data",
  caption = "VL Suppression Rate is calculated as TX_PVLS_N / TX_PVLS_D."
)
...
summary_table %>%
  kbl(
    caption = paste("Key Indicators by Country for FY", fiscal_year_selected, "Q", quarter_selected),
    align = "lcccccccc"
  ) %>%
...
## Looking at under-5 and adolescents 
...
# Function to calculate metrics and include targets
calculate_metrics <- function(data, age_group_label) {
  data %>%
    summarise(
      Age_Group = age_group_label,
      HTS_TST = sum(HTS_TST, na.rm = TRUE),
      HTS_TST_POS = sum(HTS_TST_POS, na.rm = TRUE),
      HTS_TST_target = sum(HTS_TST_targets, na.rm = TRUE),
      HTS_TST_POS_target = sum(HTS_TST_POS_targets, na.rm = TRUE),
      Positivity_Rate = HTS_TST_POS / HTS_TST,
      TX_NEW = sum(TX_NEW, na.rm = TRUE),
      Linkage_Rate = TX_NEW / HTS_TST_POS,
      TX_CURR = sum(TX_CURR, na.rm = TRUE),
      ...
      TX_PVLS_N = sum(TX_PVLS_N, na.rm = TRUE),
      VL_Suppression_Rate = TX_PVLS_N / TX_PVLS_D,
      `% HTS_TST` = HTS_TST / HTS_TST_target,
      `% HTS_TST_POS` = HTS_TST_POS / HTS_TST_POS_target
    ) %>%
    mutate_at(
      vars(Positivity_Rate, Linkage_Rate, VL_Suppression_Rate, `% HTS_TST`, `% HTS_TST_POS`),
      ~ ifelse(is.nan(.) | is.infinite(.) | is.na(.), NA, .)
    )
}


# Calculate metrics for each age group
under5_metrics <- calculate_metrics(under5_data, "Under 5")
peds_metrics <- calculate_metrics(peds_data, "Pediatric (0-14)")
adolescent_metrics <- calculate_metrics(adolescent_data, "Adolescent (10-19)")

# Combine the results
combined_metrics <- bind_rows(under5_metrics, peds_metrics, adolescent_metrics)


# Format the metrics for display
formatted_metrics <- combined_metrics %>%
  select(Age_Group, HTS_TST, HTS_TST_target, HTS_TST_POS, HTS_TST_POS_target,
         Positivity_Rate, TX_NEW, Linkage_Rate, TX_CURR, VL_Suppression_Rate) %>%
  mutate(
...
formatted_metrics %>%
  kbl(
    caption = paste("Key Metrics Comparison for", country_selected, "in FY", fiscal_year_selected, "Q", quarter_selected),
    col.names = c("Age Group", "HTS_TST", "HTS_TST Target", "HTS_TST_POS", "HTS_TST_POS Target",
                  "Positivity Rate", "TX_NEW", "Linkage Rate", "TX_CURR", "VL Suppression Rate"),
    align = "lccccccccc"
  ) %>%
...
# Set the order of Age_Group levels to Under 5, Pediatric (0-14), Adolescent (10-19)
plot_data$Age_Group <- factor(plot_data$Age_Group, levels = c("Under 5", "Pediatric (0-14)", "Adolescent (10-19)"))
...
ggplot(plot_data, aes(x = Metric, y = Value, fill = Age_Group)) +
  ...
  scale_y_continuous(
    ...
    breaks = c(0, 0.1, 0.5, 1),  # Define specific breaks
...
# Function to calculate metrics by age group and sex
calculate_metrics <- function(data, age_group_label) {
  data %>%
    group_by(sex) %>%
    summarise(
...
formatted_metrics <- combined_metrics %>%
  select(Age_Group, sex, HTS_TST, HTS_TST_target, HTS_TST_POS, HTS_TST_POS_target,
         Positivity_Rate, TX_NEW, Linkage_Rate, TX_CURR, VL_Suppression_Rate) %>%
  mutate(
    Positivity_Rate = percent(Positivity_Rate, accuracy = 0.1),
    Linkage_Rate = percent(Linkage_Rate, accuracy = 0.1),
...
formatted_metrics %>%
  kbl(
    caption = paste("Key Metrics Comparison by Sex for", country_selected, "in FY", fiscal_year_selected, "Q", quarter_selected),
...
ggplot(plot_data, aes(x = Metric, y = Value, fill = sex)) +
  ...
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
    breaks = c(0, 0.1, 0.5, 1),  # Add specific breaks for better readability
    limits = c(0, 1.1),          # Ensure all bars and labels fit within the y-axis
    expand = expansion(mult = c(0, 0.1))
  ) +
  labs(
...
## Looking at prime partners  


```{r, prime partners table, fig.width=12, fig.height=8}
...
```{r trned line plot HTS prime partner, fig.width=12, fig.height=8}
...
ggplot(trend_data, aes(x = time, y = HTS_TST_POS, color = prime_partner_name, group = prime_partner_name)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  labs(
...
ggplot(trend_data, aes(
  ...
  labs(
    ...
    y = "VL_Suppression_Rate",
    color = "Prime Partner",
    size = "TX_CURR"  # Legend for size mapping
  ) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),  # Format as percentages
    limits = c(0.50, 1.00)  # Set limits for y-axis
  ) +
...
```{r, snu1 table, fig.width=12, fig.height=8}
...
```{r trend plot snu1 HTS, fig.width=12, fig.height=8}
...
```{r snu1 trend plot viral suppression, fig.width=12, fig.height=8}
...
base_dir <- "C://Users/georg/Desktop/databases/Narratives from Panorama/"
